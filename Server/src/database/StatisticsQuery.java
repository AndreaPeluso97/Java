...
public class StatisticsQuery extends DBimplementation{
	...
	public QueryReturn statistics3(){
		...
		try {
			statistics3_stmt = conn.prepareStatement("select count(*) as ntime,utente.email,utente.nickname from manche inner join partita on manche.idpartita=partita.idpartita inner join partecipazione on partita.idpartita=partecipazione.idpartita inner join utente on partecipazione.email=utente.email group by (utente.email,utente.nickname) having count(*) >= all( select count(*) as ntime from manche inner join partita on manche.idpartita=partita.idpartita inner join partecipazione on partita.idpartita=partecipazione.idpartita inner join utente on partecipazione.email=utente.email group by (utente.email,utente.nickname)) LIMIT 1");
